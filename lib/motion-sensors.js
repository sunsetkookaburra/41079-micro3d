...
const DeviceOrientationMixin = (superclass, ...eventNames) => class extends superclass {
  ...
  [activateCallback]() {
    window.addEventListener(this[slot].eventName, this._listener, true);
  }

  [deactivateCallback]() {
    window.removeEventListener(this[slot].eventName, this._listener, true);
  }
};
...
class AbsoluteOrientationSensor extends DeviceOrientationMixin(
  ...
  [handleEventCallback](event) {
    ...
    // this.lastEuler = [heading, event.beta, event.gamma];
